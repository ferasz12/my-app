    ...
    return _db
      ...
      return qs.docs
          .map((d) {
            ...
            return LeaderboardEntry(uid: d.id, points: pts);
          })
          .where((e) => e.points > 0)
          .toList();
    });
...
class _AchievementsPageState extends State<AchievementsPage>
  ...
  // زر تجريبي لإضافة نقاط (للتطوير فقط). اجعله false في الإنتاج.
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            if (_showDebugAddPoints)
              IconButton(
                tooltip: 'زيادة نقاطي (تجريبي)',
                icon: const Icon(Icons.bolt),
                onPressed: () => _showAddMyPointsDialog(context),
              ),
          ],
        ),
        body: TabBarView(
  ...
  Future<void> _showAddMyPointsDialog(BuildContext context) async {
    final u = FirebaseAuth.instance.currentUser;
    if (u == null) return;
    ...
    final delta = await showDialog<int>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('إضافة نقاط (تجريبي)'),
        content: TextField(
          controller: c,
    ...
    await _repo.addPoints(u.uid, delta);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('تمت إضافة $delta نقطة')),
    );
  }
}
...
class _LeaderboardTabState extends State<_LeaderboardTab>
    ...
    try {
      ...
      for (var i = 0; i < list.length; i += 10) {
        ...
        for (final d in qs.docs) {
          ...
          final display = displayName.isNotEmpty
              ? displayName
              : (username.isNotEmpty
                  ? '@$username'
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<LeaderboardEntry>>(
      stream: widget.repo.watchLeaderboard(),
      builder: (context, snap) {
        if (snap.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
