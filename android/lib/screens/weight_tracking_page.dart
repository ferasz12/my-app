...
Future<_UserProfile> _loadUserProfile() async {
  ...
  final first = prefs.getString('firstName_$email') ?? '';
          ...
          build: (ctx) => pw.Directionality(
            ...
            child: pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Text('تقرير التتبّع — تطبيق وازن',
                      style: pw.TextStyle(
                        fontSize: 22,
                  ...
                  pw.Container(
                    ...
                    child: pw.Column(
                      ...
                      children: [
                        ...
                        _kv('BMR تقديري', profile.bmr != null ? '${profile.bmr!.toStringAsFixed(0)} ك.س/يوم' : 'غير متوفر'),
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Row(children: [
                    ...
                    pw.Text((profile.fullName?.isNotEmpty ?? false) ? profile.fullName! : '—'),
                  ]),
...
// جمع بيانات يومية لعدد أيام للخلف (أقدم -> أحدث)
  Future<_Series> _collectSeries({required int daysBack}) async {
    ...
    // 🔹 نجمع الأوزان من المصدرين + وزن اليوم الحالي كملحق
    ...
    // 2) القديم: weightHistory_$email => List<String(JSON)]
    ...
    // 3) لو اليوم له وزن حالي ولم يكن في الخريطة
    ...
    for (int i = daysBack - 1; i >= 0; i--) {
      ...
      double cal = 0.0;
// استخدم القيم المُثبّتة لنهاية اليوم فقط إن وُجدت (diet_YYYY-MM-DD)
final _finalKey = 'diet_' + key;
final _finalRaw = prefs.getString(_finalKey);
final _todayYmd = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day).toIso8601String().split('T').first;
if (_finalRaw != null) {
  try {
    final _m = jsonDecode(_finalRaw) as Map<String, dynamic>;
    cal = (_m['calories'] as num?)?.toDouble() ?? 0.0;
  } catch (_) {}
} else {
  // لو اليوم الحالي ولم يتم تثبيته بعد ⇒ اعتبره 0 في التقارير/الـ PDF
  if (key != _todayYmd) {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _exportTrackingPdf(context),
          ),
        ],
      ),
      body: TabBarView(
...
String _smartAnalysis(
  ...
  switch (goal) {
    ...
    case 'خفض الدهون':
      if (rF > 1.0 && rC > 1.0) {
        return 'هدفك إنقاص/خفض الدهون: $highFat$highCarbخفّف الدهون والمقليات والسكريات، وارفع البروتين والخضار. $okCal';
      }
      if (rF > 1.0) {
        return 'هدفك إنقاص/خفض الدهون: $highFatاختر مصادر بروتين خفيفة وقلّل الزيوت. $okCal';
      }
      if (rC > 1.2) {
        return 'هدفك إنقاص/خفض الدهون: $highCarbقلّل النشويات المكررة وزد ألياف الخضار. $okCal';
      }
      if (rP < 0.8) {
        return 'هدفك إنقاص/خفض الدهون: $lowProteinزيد حصتك من البروتين للحفاظ على الكتلة العضلية.';
      }
      return 'جيد! توزيعتك تدعم الهدف — استمر على توازن بروتين أعلى ودهون/كارب مضبوطة. $okCal';

    case 'زيادة الوزن':
      if (rCal < 0.95) {
        return 'لهدف زيادة الوزن: $lowCal زد حصصك تدريجيًا خاصة الكارب المعقّد والبروتين.';
      }
      if (rP < 0.9) {
        return 'لهدف زيادة الوزن: $lowProteinاحرص على بروتين كافٍ مع كل وجبة.';
      }
      if (rC < 0.9) {
        return 'لهدف زيادة الوزن: الكارب أقل من المطلوب — زِد الأرز/الخبز الكامل/الشوفان.';
      }
      return 'ممتاز! تقدّم مناسب للزيادة — حافظ على فائض سعرات متزن وبروتين كافٍ.';
    ...
    case 'بناء العضلات':
      ...
      if (rCal < 0.95) {
        return 'هدفك بناء العضلات: $lowCal زد السعرات قليلاً مع توزيع كارب جيد حول التمرين.';
      }
    ...
    case 'تحسين الصحة العامة':
      if (rF > 1.2) {
        return 'نمط صحي: $highFatقلّل المقليات واختر دهونًا صحية بكميات معتدلة.';
      }
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children:[Icon(Icons.local_fire_department, size: 18, color: cs.primary), const SizedBox(width:6), Text('ضمن السعرات: $_okCalDays يوم', style: t.bodyMedium)]),
            const SizedBox(height: 6),
            Row(children:[Icon(Icons.egg_alt_rounded, size: 18, color: Colors.indigo), const SizedBox(width:6), Text('البروتين مُتحقق: $_okPDays يوم', style: t.bodyMedium)]),
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 8, runSpacing: 8, children: [
            legend(_calColor, 'السعرات (ك.س)'),
            legend(Colors.indigo, 'البروتين (×4 ك.س)'),
            legend(Colors.orange, 'الكارب (×4 ك.س)'),
            legend(Colors.redAccent, 'الدهون (×9 ك.س)'),
          ]),
          const SizedBox(height: 6),
          ...
          Row(children: [
            Switch(
              ...
              onChanged: (v) async {
                ...
                await _saveMicroEnabled(_prefsRef!, _emailRef);
                _computeMicroProgress();
              },
            ),
            const SizedBox(width: 6),
...
class _WeightTabState extends State<_WeightTab> with WidgetsBindingObserver {
  ...
  Future<void> _loadWeights() async {
    ...
    // تحويل إلى نقاط مرتبة
    final dates = map.keys.toList()..sort();
    final pts = <_WeightPoint>[];
    for (final d in dates) {
      pts.add(_WeightPoint(DateTime.parse(d), map[d]!));
    }
        ...
        children: [
          ...
          if (currentWeight != null)
            Padding(
              ...
              child: Text(
                  'وزنك الحالي: ${currentWeight!.toStringAsFixed(1)} كجم',
                  style: const TextStyle(fontWeight: FontWeight.w600)),
            ),
          if (avg > 0)
            Padding(
              padding: const EdgeInsets.only(top: 4.0),
              child: Text('متوسط آخر أسبوع: ${avg.toStringAsFixed(1)} كجم',
                    ...
                    LineChartData(
                      ...
                      lineBarsData: [
                        LineChartBarData(
                          ...
                          spots: points
                              .map((e) => FlSpot(
                                    e.t.millisecondsSinceEpoch.toDouble(),
                                    e.kg,
                                  ))
                              .toList(),
...
Widget build(BuildContext context) {
  ...
  return Directionality(
    textDirection: ui.TextDirection.rtl, // ✅ Flutter TextDirection بدون تعارض
    child: Card(
      elevation: 2,
...
class _InsightsTabState extends State<_InsightsTab> {
  ...
  Future<void> _loadAll() async {
    ...
    weightKg   = prefs.getDouble('weight_$email') ?? prefs.getDouble('current_weight_$email');
    ...
    for (int i = 0; i < 7; i++) {
      ...
      final ymd = DateTime(day.year, day.month, day.day).toIso8601String().split('T').first;

      // حمولة الأكل – جرّب عدة مفاتيح محتملة حسب أنماط التخزين
      List entries = [];
      for (final k in [
        'intake_entries_${email}_$ymd',
        'kcal_entries_${email}_$ymd',
        ...
        if (raw != null) {
          try {
            ...
            if (v is List && v.isNotEmpty) { entries = v; break; }
          } catch (_) {}
        }
      }
      final sums = sumFromIterable(entries);
      final kcal = (sums['cal'] ?? sums['k'] ?? 0.0) as double;

      // الماء
      int water = 0;
      for (final k in [
        'water_${email}_$ymd',
        'water_ml_${email}_$ymd',
        'water_$ymd'
      ]) {
        final vInt = prefs.getInt(k);
        final vD = prefs.getDouble(k);
        if (vInt != null) { water = vInt; break; }
        if (vD != null) { water = vD.toInt(); break; }
      }
      // الخطوات (قراءة من التخزين إن وجدت)
      int steps = 0;
      for (final k in [
        'steps_${email}_$ymd',
        'steps_$ymd',
        ...
        final vInt = prefs.getInt(k);
        if (vInt != null) { steps = vInt; break; }
      }
      last7.add(_DaySummary(date: DateTime(day.year, day.month, day.day), kcal: kcal, waterMl: water, steps: steps));
    }
    setState(() {});
  }
  ...
  String _bmiLabel(double v) {
    if (v < 18.5) return 'نحيف';
    if (v < 25) return 'متناسق';
    if (v < 30) return 'زائد';
    return 'سمنة';
  }

  // تقييم الالتزام بالسعرات خلال 7 أيام
          ...
          child: PageView(
            ...
            children: [
              ...
              _InsightCard(
                ...
                subtitle: targetCal == null ? 'لا يوجد هدف سعرات' : 'هدفك: ${targetCal!.toStringAsFixed(0)} ك.س',
...
class _DaySummary {
  ...
  final int waterMl;
  final int steps;
  _DaySummary({required this.date, required this.kcal, required this.waterMl, required this.steps});
}
