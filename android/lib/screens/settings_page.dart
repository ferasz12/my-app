...
// ✅ صفحة تعديل اليوزر
import 'package:my_app/settings/edit_username_page.dart';
...
class SettingsPage extends StatelessWidget {
  ...
  Future<void> _logout(BuildContext context) async {
    ...
    try {
      // Firebase signOut (لو ما كان مُهيأ، التجاهل في catch)
  ...
  Future<void> _deleteAccount(BuildContext context) async {
    ...
    try {
      ...
      // 1) Firestore: حذف مجموعات فرعية شائعة + الوثيقة الجذرية
      if (uid != null) {
        await _deleteUserFirestore(uid);
      }

      // 2) Storage: حذف ملفات المستخدم
      if (uid != null) {
        await _deleteUserStorage(uid);
      }

      // 3) SharedPreferences: مسح مفاتيح المستخدم
      await _wipeLocalPrefs(uid: uid, email: email);

      // 4) حذف مستخدم المصادقة (لو يلزم إعادة مصادقة، يفضل معالجتها في شاشة منفصلة)
      if (user != null) {
        try {
          await user.reload();
          await user.delete();
        } on FirebaseAuthException catch (e) {
          // لو احتاج إعادة مصادقة، كبّر الرسالة للمستخدم
          debugPrint('delete auth user failed: ${e.code}');
        }
      }

      // 5) تسجيل خروج احتياطي
            ...
            _sectionCard(
              ...
              children: [
                _tile(
                  ...
                  subtitle: 'الاسم واليوزر والبريد والصورة',
                  onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ProfilePage())),
                ),
                _tile(
                  context: context,
                  icon: Icons.tag,
                  title: 'اسم المستخدم (اليوزر)',
                  subtitle: 'يمكن تغييره كل 7 أيام',
                  onTap: () async {
                    ...
                    if (result is String && context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('تم تحديث اليوزر إلى @$result')),
                      );
                    }
                  },
                ),
            ...
            _sectionCard(
              ...
              children: [
                ...
                _tile(
                  ...
                  subtitle: '@app / t.me/app / instagram.com/app',
                  onTap: () => _openUrl('https://instagram.com/app'),
                ),
              ],
            ),
