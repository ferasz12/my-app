...
class UserRepository {
  ...
  void _validateHandle(String handle) {
    // قواعد اليوزرنيم الموحدة في التطبيق:
    // - 5 إلى 20 حرفًا
    // - إنجليزي فقط (a-z) + أرقام
    // - يبدأ بحرف
    // - بدون مسافات أو رموز
    if (handle.isEmpty) throw Exception('اسم المستخدم غير صالح');
    if (handle.length < 5) throw Exception('اسم المستخدم لازم يكون ٥ أحرف أو أكثر');
    if (handle.length > 20) throw Exception('اسم المستخدم طويل جدًا');

    final ok = RegExp(r'^[a-z][a-z0-9]{4,19}$').hasMatch(handle);
    if (!ok) {
      throw Exception('اسم المستخدم لازم يبدأ بحرف ويكون إنجليزي فقط (حروف/أرقام)');
    }
  }
  ...
  Future<void> updateUsername({required String username}) async {
    ...
    await _db.runTransaction((tx) async {
      // ✅ Firestore Transaction rule: execute ALL reads before ANY writes.
      // 1) Read new username reservation doc
      final newSnap = await tx.get(newHandleRef);

      // 2) Read current user doc (to get old username)
      final userSnap = await tx.get(userRef);
      final userData = userSnap.data() ?? <String, dynamic>{};
      final oldHandle = (userData['username'] as String?)?.trim();
      final oldDisplayName = (userData['displayName'] as String?)?.trim();

      // 3) If user had an old username, read its reservation doc BEFORE writes
      ...
      if (oldHandle != null && oldHandle.isNotEmpty && oldHandle != handle) {
        oldRef = _usernameDoc(_normalizeHandle(oldHandle));
        oldSnap = await tx.get(oldRef);
      }

      // ---- After this point: WRITES only ----
      if (newSnap.exists) {
      ...
      // لا نغيّر displayName إلا إذا كان فارغًا، لأن الاسم الحقيقي (name) قد يكون مختلفًا عن username.
      final shouldUpdateDisplayName =
          (oldDisplayName == null || oldDisplayName.isEmpty);
      ...
      // Free old handle if it belonged to this user
      if (oldRef != null && oldSnap != null) {
