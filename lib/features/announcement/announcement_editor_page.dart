...
class _AnnouncementEditorPageState extends State<AnnouncementEditorPage> {
  ...
  Future<void> _pickDateTime({required bool isStart}) async {
    ...
    // Keep the previous logic (start = _startAt or now, end = _endAt or +7 days),
    // but clamp the *date* used by showDatePicker so it never violates
    // initialDate >= firstDate and initialDate <= lastDate.
    final initialDt = isStart
        ? (_startAt ?? now)
        : (_endAt ?? now.add(const Duration(days: 7)));

    final firstDate = DateTime(now.year, now.month, now.day).subtract(const Duration(days: 1));
    final lastDate = DateTime(now.year, now.month, now.day).add(const Duration(days: 365));

    var initialDate = DateTime(initialDt.year, initialDt.month, initialDt.day);
