...
class FoodAiService {
  ...
  // ===== Gemini direct (اختياري) =====
  static const String _geminiBase = 'https://generativelanguage.googleapis.com/v1beta';
  static const String _geminiDefaultModel = 'gemini-2.5-flash'; // يمكن ضبط GEMINI_MODEL في .env
  ...
  Future<FoodAnalysis> analyzeFile(File imageFile, {String? clarifier}) async {
    ...
    if (geminiKey.isNotEmpty) {
      final direct = await _analyzeFileWithGemini(
        imageFile,
        apiKey: geminiKey,
        clarifier: clarifier,
      );
  ...
  Future<FoodAnalysis> analyzeImageUrl(String imageUrl, {String? clarifier}) async {
    final geminiKey = (dotenv.env['GEMINI_API_KEY'] ?? '').trim();
    if (geminiKey.isNotEmpty) {
      final direct = await _analyzeUrlWithGemini(
        imageUrl,
        apiKey: geminiKey,
        clarifier: clarifier,
      );
  ...
  /// تحليل نصّي مباشر عبر Gemini (إضافي)
  Future<FoodAnalysis> analyzeText(String description, {String? clarifier}) async {
    final geminiKey = (dotenv.env['GEMINI_API_KEY'] ?? '').trim();
    if (geminiKey.isEmpty) {
      throw Exception('GEMINI_API_KEY غير موجود. أضفه في .env أو استخدم endpoint الحالي للصور فقط.');
    }
    final direct = await _analyzeTextWithGemini(
      description,
      apiKey: geminiKey,
      clarifier: clarifier,
    );
    if (direct == null || !direct.isValid) {
      throw Exception('تعذر التحليل عبر Gemini.');
    }
    return direct;
  }

  String _resolvedGeminiModel() {
    final v = (dotenv.env['GEMINI_MODEL'] ?? '').trim();
    return v.isNotEmpty ? v : _geminiDefaultModel;
  }

  Future<FoodAnalysis?> _analyzeFileWithGemini(
    File imageFile, {
    ...
    try {
      ...
      return _analyzeBytesWithGemini(bytes, apiKey: apiKey, clarifier: clarifier);
    } catch (e) {
      debugPrint('[Gemini] analyzeFile failed: $e');
      return null;
    }
  }

  Future<FoodAnalysis?> _analyzeUrlWithGemini(
    String imageUrl, {
    required String apiKey,
    String? clarifier,
  }) async {
    try {
      final r = await http.get(Uri.parse(imageUrl)).timeout(const Duration(seconds: 20));
      if (r.statusCode >= 400) return null;
      return _analyzeBytesWithGemini(r.bodyBytes, apiKey: apiKey, clarifier: clarifier);
    } catch (e) {
      debugPrint('[Gemini] analyzeImageUrl failed: $e');
      return null;
    }
  }

  Future<FoodAnalysis?> _analyzeTextWithGemini(
    String description, {
    required String apiKey,
    String? clarifier,
  }) async {
    return _callGemini(
      ...
      parts: [
        {
          "text": _buildTextPrompt(description, clarifier: clarifier),
        }
      ],
    );
  }

  String _buildTextPrompt(String description, {String? clarifier}) {
    final c = (clarifier != null && clarifier.trim().isNotEmpty)
        ? "\nClarifier: ${clarifier.trim()}"
        : "\nClarifier: (none)";
    return "حلّل وصف الوجبة التالي وقدّر السعرات والماكروز. أعد JSON فقط.\nوصف: ${description.trim()}$c";
  }

  Future<FoodAnalysis?> _analyzeBytesWithGemini(
    Uint8List bytes, {
    required String apiKey,
    String? clarifier,
  }) async {
    ...
    final parts = <Map<String, dynamic>>[
      {
        "text": "حلّل صورة الطعام وقدّر السعرات والماكروز. أعد JSON فقط.${(clarifier != null && clarifier.trim().isNotEmpty) ? "\nClarifier: ${clarifier.trim()}" : ""}"
      },
      {
        "inline_data": {"mime_type": "image/jpeg", "data": b64}
      }
    ];
    return _callGemini(apiKey: apiKey, parts: parts);
  }

  Future<FoodAnalysis?> _callGemini({
    required String apiKey,
    required List<Map<String, dynamic>> parts,
  }) async {
    final model = _resolvedGeminiModel();
    ...
    final responseJsonSchema = {
      ...
      "properties": {
        "label": {"type": "string"},
        "serving": {"anyOf": [{"type": "string"}, {"type": "null"}]},
        "serving_size_g": {"anyOf": [{"type": "number", "minimum": 0}, {"type": "null"}]},
        "calories": {"type": "number", "minimum": 0},
        "protein": {"type": "number", "minimum": 0},
        "carbs": {"type": "number", "minimum": 0},
        "fat": {"type": "number", "minimum": 0},
        "ingredients": {"anyOf": [{"type": "array", "items": {"type": "string"}}, {"type": "null"}]},
        "confidence": {"anyOf": [{"type": "number", "minimum": 0, "maximum": 1}, {"type": "null"}]}
      }
    };

    final uri = Uri.parse('$_geminiBase/models/$model:generateContent?key=$apiKey');
    final body = jsonEncode({
      "systemInstruction": {
        "parts": [
          {"text": "أنت خبير تغذية. أعد JSON فقط."}
        ]
      },
      "contents": [
    ...
    final decoded = jsonDecode(resp.body);
    final raw = _extractJsonFromGemini(decoded);
    if (raw == null) return null;

    final result = FoodAnalysis.fromResponse(raw);
    return result.isValid ? result : null;
  }

  Map<String, dynamic>? _extractJsonFromGemini(dynamic payload) {
    try {
        ...
        if (parts != null) {
          for (final p in parts) {
            ...
            if (t is String && t.trim().isNotEmpty) {
              return _extractFirstJsonObject(t);
            }
          }
        }
      }
