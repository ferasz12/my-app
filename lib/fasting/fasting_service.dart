...
class FastingService extends ChangeNotifier {
  ...
  /// بدء الصيام + جدولة الإشعارات.
  /// مهم: نفعّل المؤقت ونحدّث الواجهة فورًا قبل جدولة الإشعارات،
  /// لأن جدولة إشعار البداية عند DateTime.now قد ترمي خطأ على بعض الأجهزة.
  Future<void> startFasting({
    ...
    // الإشعارات لا يجب أن تمنع المؤقت من العمل.
    try {
      ...
      // بداية الصيام: إذا البداية الآن أو في الماضي نعرض إشعار فوري بدل جدولة وقت فات.
      if (!start.isAfter(now.add(const Duration(seconds: 10)))) {
        await FastingNotifications.instance.showNow(
          id: 1001,
          title: 'بدأ الصيام',
      ...
      if (mid.isAfter(now.add(const Duration(seconds: 10)))) {
        await FastingNotifications.instance.scheduleOnce(
          id: 1002,
          title: 'منتصف المدة',
      ...
      if (end.isAfter(now.add(const Duration(seconds: 10)))) {
        await FastingNotifications.instance.scheduleOnce(
          id: 1003,
          title: 'انتهاء الصيام',
      ...
      final mealReminder = end.add(const Duration(minutes: 10));
      if (mealReminder.isAfter(now.add(const Duration(seconds: 10)))) {
        await FastingNotifications.instance.scheduleOnce(
          id: 1004,
          title: 'تذكير وجبة',
      ...
      debugPrint('⚠️ Fasting notifications scheduling skipped: $e');
    }
