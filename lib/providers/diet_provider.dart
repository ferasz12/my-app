...
class DietProvider with ChangeNotifier {
  ...
  Future<void> loadActiveDiet() async {
    ...
    final k = 'activeDietId_$storageKey';

    // ✅ Migration: المفتاح القديم كان عام
    final legacy = prefs.getString('activeDietId');
    if (legacy != null && prefs.getString(k) == null) {
      await prefs.setString(k, legacy);
      await prefs.remove('activeDietId');
    }

    final id = prefs.getString(k);
    if (id != null) {
  ...
  Future<void> setActiveDietById(String? id) async {
    ...
    if (id == null) {
      ...
      await prefs.setString(k, _activeDiet!.id);
    }
    notifyListeners();
  }
