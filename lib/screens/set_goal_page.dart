...
class _SetGoalPageState extends State<SetGoalPage> {
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    hintText: 'مثال: ابي انزل 5 كيلو خلال شهر ',
  ...
  Future<void> _handleSave(BuildContext context) async {
    ...
    if (widget.embedded) {
      ...
      // ✅ مهم: نستخدم push بدل pushReplacement حتى تقدر ترجع وتعدّل هدفك.
      // ✅ بعد تحديد الهدف: نعرض صفحة تحفيزية (رسم مسار الوزن) ثم ننتقل للملخص.
      Navigator.pushNamed(context, '/goal-progress', arguments: built);
    }
  ...
  Future<void> _computeAndStoreMacros() async {
    ...
    final String effectiveGoal = (goalFatShred || goal.trim() == 'تنشيف الدهون')
        ? 'تنشيف الدهون'
        : goal;
    final String macroPlanId = MacroPlanEngine.defaultPlanIdForGoal(effectiveGoal);
    final List<MacroPlanOption> planOptions = MacroPlanEngine.buildOptions(
      goal: effectiveGoal,
    ...
    final MacroPlanOption selectedPlan = planOptions.firstWhere(
      (o) => o.id == macroPlanId,
      orElse: () => planOptions.first,
    );

    final double targetCalories = selectedPlan.calories;
    ...
    await prefs.setDouble('caloriesNeeded_$email', targetCalories);
    ...
    try {
      ...
      // Ignore: local values will still be used
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final cs = Theme.of(context).colorScheme;
    return Scaffold(
      // ✅ خلفية فخمة مناسبة لتطبيق صحي
      resizeToAvoidBottomInset: true,
      appBar: AppBar(
        title: const Text('ضع هدفك'),
        centerTitle: true,
        elevation: 0,
...
class _AnalysisCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String riskNote = () {
      switch (risk.label) {
        case 'منخفض':
          return 'معدل أسبوعي واقعي وآمن .';
        case 'متوسط':
          return 'معدل سريع لكن ممكن.';
        case 'مرتفع':
          return 'معدل عالي؛ يُستحسن إطالة المدة لتقليل الإجهاد  .';
        default:
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const Text('تنبيه:  هذي تقديرات تقريبية وليست نصيحة طبية.'),
        ],
      ),
    );
  }
