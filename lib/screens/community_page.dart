...
class _ComposePostSheetState extends State<_ComposePostSheet> {
  ...
  void _wrapSelection(String start, String end) {
    ...
    final safeStart = selection.isValid ? selection.start : text.length;
    final safeEnd = selection.isValid ? selection.end : text.length;
    final a = safeStart < 0 ? text.length : safeStart;
    final b = safeEnd < 0 ? text.length : safeEnd;
    final selected = a == b ? 'النص' : text.substring(a, b);
    final next = text.replaceRange(a, b, '$start$selected$end');
    _controller.value = TextEditingValue(
      text: next,
      selection: TextSelection.collapsed(
          offset: a + start.length + selected.length + end.length),
    );
  }
  ...
  void _insertAtCursor(String value) {
    ...
    final safe = offset < 0 ? text.length : offset;
    final next = text.replaceRange(
        safe, selection.isValid ? selection.end : safe, value);
    _controller.value = TextEditingValue(
      text: next,
      selection: TextSelection.collapsed(offset: safe + value.length),
    );
  }
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'اكتب نصًا فقط. تقدر تستخدم تنسيق بسيط مثل العريض أو النقاط.',
              ...
              Container(
                decoration: BoxDecoration(
                  ...
                  border:
                      Border.all(color: cs.outlineVariant.withOpacity(0.65)),
                ),
                child: Column(
                  children: [
                    Padding(
                    ...
                    Padding(
                      ...
                      child: TextField(
                        ...
                        decoration: const InputDecoration(
                          border: InputBorder.none,
                          hintText: 'اكتب سؤالك، معلومتك، أو تجربتك هنا...',
                          counterText: '',
                        ),
                      ),
                    ),
...
class _RichCommunityText extends StatelessWidget {
  ...
  List<TextSpan> _spans(String input, TextStyle base) {
    ...
    for (final m in regex.allMatches(input)) {
      ...
      spans.add(TextSpan(
          text: boldText, style: base.copyWith(fontWeight: FontWeight.w900)));
      last = m.end;
    }
